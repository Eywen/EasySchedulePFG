...
/**
 * Performance test for the Asignatura entity.
 */
class AsignaturaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Asignatura entity")
        ...
        .repeat(2) {
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_asignatura_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created asignatura")
                .get("${new_asignatura_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created asignatura")
            .delete("${new_asignatura_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
